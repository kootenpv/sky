...
class MoneyMatcher():
    def __init__(self, locale = None): 
        ...
        self.currency = re.compile(number_regex + optional_space + units + optional_space + locale['currency'], re.IGNORECASE) 
    ...
    def find(self, text, min_amount = 0, max_amount = 10**12):
        ...
        results = [(text[x[0].start:x[0].stop], self.convertMatchToValue(x[1]), x[0].start, x[0].stop) for x in matches]
...
def investment_annotation(title, body, money, entities, indicators = None, max_character_distance = 100): 
    ...
    entities_locations = [(e['text'], total_content.find(e['text'])) for e in entities if e['type'] in ['Person', 'Company']] 
    indicator_locations = [(x, total_content.find(x)) for x in indicators if total_content.find(x) > -1]
    me_combinations = {}
    for m in money:
        for e in entities_locations:
            for i in indicator_locations:
                if abs(e[1] - m[1]) < max_character_distance and abs(e[1] - i[1]) < max_character_distance:
